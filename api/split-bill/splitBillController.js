...
const createItSplit = async (req, res) => {
    try {
        ...
        const checkQuantityQuery = `SELECT split_quantity FROM order_items WHERE OrderID = ?`;
        ...
        for (const item of items) {
            ...
            if (itemDetails) {
                ...
                const checkQuantityResult = await poolConnection.query(checkQuantityQuery, [orderId]);
                const checkQuantity = checkQuantityResult[0].split_quantity;
